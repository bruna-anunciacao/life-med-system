      ...
      <CardContent className={`p-5 flex gap-5 items-center flex-row! items-center!`}>
        ...
        <div className="flex flex-col gap-2 flex-shrink-0">
          {(appt.status === "CONFIRMED" || appt.status === "PENDING") && (
            <>
              <Button size="sm" className="px-4 py-1.5 rounded-lg bg-[#006fee] font-semibold text-xs text-white cursor-pointer transition-all duration-200 hover:bg-[#0056b3]">Detalhes</Button>
              <Button size="sm" className="px-4 py-1.5 border border-red-200 rounded-lg bg-white font-semibold text-xs text-red-500 cursor-pointer transition-all duration-200 hover:bg-red-50" onClick={() => onCancel(appt.id)}>Cancelar</Button>
            </>
          )}
          {appt.status === "COMPLETED" && (
            <Button size="sm" className="px-4 py-1.5 rounded-lg bg-[#006fee] font-semibold text-xs text-white cursor-pointer transition-all duration-200 hover:bg-[#0056b3]">Ver Detalhes</Button>
          )}
          {appt.status === "CANCELLED" && (
            <Button size="sm" className="px-4 py-1.5 border border-gray-200 rounded-lg bg-white font-semibold text-xs text-gray-700 cursor-pointer transition-all duration-200 hover:bg-gray-50" onClick={onRebook}>
              Reagendar
            </Button>
          )}
        </div>
