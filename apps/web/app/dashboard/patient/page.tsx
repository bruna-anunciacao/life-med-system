...
const PatientDashboard = () => {
  ...
  const stats = [
    { title: "Próximas Consultas", value: "2", icon: <CalendarIcon />, color: "primary" },
    { title: "Aguardando Confirmação", value: "1", icon: <ClockIcon />, color: "warning" },
    { title: "Consultas Realizadas", value: "5", icon: <CheckIcon />, color: "success" },
  ];
  ...
  return (
    <section className={styles.container}>
      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>{userName ? `Olá, ${userName}!` : "Painel do Paciente"}</h1>
          <p className={styles.subtitle}>Acompanhe suas consultas e encontre profissionais de saúde.</p>
        </div>
        ...
        {stats.map((stat, index) => (
          <Card key={index} className={styles.statCard}>
            <CardContent className={styles.statCardBody}>
              ...
              <div className={`${styles.iconWrapper} ${styles[stat.color]}`}>{stat.icon}</div>
            </CardContent>
          </Card>
        ))}
      </div>
          ...
          <Card className={styles.nextApptCard}>
            <CardContent className={styles.nextApptBody}>
              <div className="flex items-center gap-4">
                <div>
                  <h3 className={styles.nextApptName}>{nextAppointment.doctorName}</h3>
                  <p className={styles.nextApptSpecialty}>{nextAppointment.specialty}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <Badge className={styles.modalityChip}>
                      {nextAppointment.type === "VIRTUAL" ? "Consulta Online" : "Presencial"}
                    </Badge>
            ...
            <CardContent>
              {upcomingAppointments.map((appt) => (
                <div key={appt.id} className={styles.appointmentRow}>
                  ...
                  <Badge className={`${styles.statusChip} ${appt.status === "CONFIRMED" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
            ...
            {suggestedDoctors.map((doc) => (
              <Card key={doc.id} className={styles.doctorCard}>
                <CardContent className={styles.doctorCardBody}>
                  <div className={styles.avatarNoPhoto}>{doc.name.charAt(0).toUpperCase()}</div>
                  <div className="flex-1">
                    <p className={styles.doctorName}>{doc.name}</p>
                    <p className={styles.doctorSpecialty}>{doc.specialty}</p>
                  </div>
                  <Button size="sm" className={styles.viewProfileButton} onClick={() => router.push("/dashboard/patient/search")}>
                    Agendar
                  </Button>
                </CardContent>
              </Card>
